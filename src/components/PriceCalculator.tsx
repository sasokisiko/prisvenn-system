...
type ProjectType = 'newWebsite' | 'newWebshop' | 'updateExisting';
type SpecialFeatures = 'yes' | 'no' | 'unknown';
type PageCount = '1-5' | '5-10' | '10+' | 'unknown';

const PriceCalculator: React.FC = () => {
  const navigate = useNavigate();
  const [currentStep, setCurrentStep] = useState(1);
  const [projectType, setProjectType] = useState<ProjectType | null>(null);
  const [specialFeatures, setSpecialFeatures] = useState<SpecialFeatures | null>(null);
